...
.claude/
  settings.json                    # enables optional agent-teams mode + defaults
  agents/
...
docs/agents/
  board.md                         # portfolio board (epics + tasks)
  epics/                           # epic plans for large features/modules
...
## Scaling to large features and many tasks

- **Large feature/module** → tell the PM it's large; it writes an **epic plan**
  (`docs/agents/epics/<feature>.md`) of shippable **phases**, you approve the
  breakdown, then run one phase at a time (each its own PR). The architect
  re-reads shipped code before each phase.
- **Several tasks at once** → run **separate PM sessions**, one per task, each on
  its own git worktree/branch; monitor them with `claude agents`. Keep to a WIP
  limit of ~2–3.
- **Cross-layer build or parallel review** → optional **agent-teams** mode
  (enabled in `.claude/settings.json`); teammates coordinate but cost more tokens.

Full commands and examples are in `GUIDE.md` (Part 5). `.claude/settings.json`
turns on agent teams and sets `teammateMode: in-process` so it works in any
terminal; remove the env line to disable team mode.
