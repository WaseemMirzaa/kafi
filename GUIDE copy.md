...
## Part 5 — Scaling: large features & multiple tasks at once

### Large feature or whole module (an "epic")

Don't scope a big feature as one task. Tell the PM it's large and ask for a phase
breakdown first:

```
Project: Nannies app
Feature: full nanny profile module (build, view, availability, rate).
This is large — read the code and break it into phases first. Don't build yet.
```

The PM reads the codebase, writes an epic plan to `docs/agents/epics/<feature>.md`
with phases and dependencies, and asks you to approve the breakdown. Then run one
phase at a time:
...
```
```
Phase 1 is merged. Start Phase 2.
```

Each phase is its own scope → plan → build → review → PR cycle, and the architect
re-reads the shipped code before planning the next — so phases build on reality.
Within a phase, say "split across parallel developers where safe" to run several
developers at once (each isolated in its own git worktree, merged before review).

### Several tasks at the same time

Best run as **separate sessions**, one per task, each on its own branch/worktree
— this keeps each context clean and prevents file collisions. A simple flow:

```bash
# task A
git worktree add ../repo-side-cart -b fix/naturesecret-side-cart
cd ../repo-side-cart && claude --agent project-manager

# task B (another terminal/tab)
git worktree add ../repo-nanny-edit -b feat/nanny-profile-edit
cd ../repo-nanny-edit && claude --agent project-manager
```

Monitor everything running in the background from one screen:

```bash
claude agents     # agent view: every session, its state, what needs you
```

Keep concurrent tasks to ~2–3 (the WIP limit) — past that, review quality and
your attention drop and "throughput" is illusory. The portfolio board
(`docs/agents/board.md`) tracks epics, tasks, and branches across all of it.

### Optional: agent-teams mode (cross-layer or parallel review)

For one big phase where frontend + backend + tests must coordinate, or to run a
parallel multi-lens review, the PM can spawn an **agent team** (teammates that
talk to each other and share a task list). It's already enabled in
`.claude/settings.json`. Just ask:

```
Build Phase 2 as an agent team: one teammate on the form, one on the schedule
picker, one on tests. Have them coordinate.
```
```
Review PR #42 as a team: one on security, one on performance, one on tests.
```

Notes:
- Agent teams use **significantly more tokens** than subagents — use them when
  coordination (not just parallelism) adds value.
- Only **one team at a time** per session.
- Teammates read `CLAUDE.md` (so the quality bar, read-first, and design-system
  rules still apply), but the agents' preloaded `skills:` do **not** carry into
  team mode — the standards come from `CLAUDE.md` there.
- On macOS you can get a separate pane per teammate by installing tmux
  (`brew install tmux`) and launching inside it; otherwise it runs in-process
  (cycle teammates with Shift+Down). To disable team mode, remove the env line
...
/agents         # subagents in THIS session (Running tab) + Library
/tasks          # background items in this session — attach/stop
Ctrl+B          # background the running task and keep typing
/rename         # name a session so it's identifiable in agent view
```
