...
```
docs/agents/
  board.md                         # portfolio board (epics + tasks), PM owns it
  epics/<feature>.md               # epic plan: phases + status, for large work
...
**Epics and phases** use a lighter status set tracked in the epic doc:
`DRAFT → APPROVED → IN_PROGRESS → DONE` for the epic, and per phase
`PLANNED → ACTIVE → MERGED`. A phase becoming `ACTIVE` spins up a normal task
that runs the full task status flow above.
...
## 6. Scaling: large work & concurrency

### Work hierarchy
Decompose big work so no single task is too large to plan well or review safely:

```
Epic     a whole feature or module           docs/agents/epics/<feature>.md
  Phase  a shippable slice (own PR)           one scope/plan/build/review cycle
    Task a unit within a phase                 the standard task slug
      Work unit a file-set a developer owns    INDEPENDENT or SEQUENTIAL
```

A bug or small change is just a single Task. A feature/module is an Epic the PM
splits into Phases; each Phase runs the full scope → plan → build → review → PR
cycle. Phases are sequential (the architect re-reads shipped code before planning
the next). Work units inside a phase can run in parallel.

### Two axes of parallelism
- **Within one task/phase (default, cheap, stable):** the PM spawns parallel
  **background developer subagents**, each in its own **git worktree**, on
  non-overlapping work units, then merges. Use this for most parallel work.
- **Across separate tasks at once (portfolio):** run **one PM session per task**,
  each on its own branch/worktree, and monitor them together with `claude agents`
  (agent view). Separate sessions keep contexts clean and avoid collisions.
- **Optional — agent teams (experimental, token-heavy):** for one large phase
  where cross-layer pieces must coordinate (frontend + backend + tests talking to
  each other) or for parallel multi-lens review, the PM may run an agent team.
  One team per lead at a time. Reserve it for cases where coordination — not just
  parallelism — adds value, because it costs significantly more tokens.

### Concurrency safety (non-negotiable)
- **One branch/worktree per task.** Never two writers on the same file.
- **Unique slugs.** Each task owns its own scope/plan/build/review files, so
  concurrent tasks never write the same artifact.
- **WIP limit.** Keep concurrent tasks to a manageable number (≈2–3) so review
  quality and your attention don't degrade. Throughput past that is illusory.
- **State lives in files, not context.** Epic/phase/task status is tracked in
  `docs/agents/` so any session (or a fresh one) can pick up where things stand.

### Context budget
Large work fails when the orchestrator's context fills with detail. The PM
delegates all verbose work (search, test runs, logs) to subagents and consumes
only summaries; the epic doc and board carry state across context resets and
auto-compaction.

## 7. Role skills (auto-loaded)
